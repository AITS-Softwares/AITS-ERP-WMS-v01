...
export default function WmsSalesDispatchPage() {
  ...
  async function submitOrder() {
    try {
      setSubmittingOrder(true); setNotice(null);
      const response = await fetch(`/api/wms/sales-orders/${encodeURIComponent(order.name)}/submit`, { method: "POST", headers: authHeaders() });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.message || "Unable to submit Sales Order.");
      await selectOrder(order.name);
      setNotice({ tone: "success", message: payload.message });
      loadOrders();
    } catch (error) { setNotice({ tone: "error", message: error.message || "Unable to submit Sales Order." }); }
  ...
  const draft = Number(order?.docstatus) === 0;
